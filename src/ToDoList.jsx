...
function ToDoList() {
    ...
    const isDisabled = tasks.length > 2 || tasks.includes(newTask) || newTask.trim() == "";
        ...
        <div className="text-center">
            ...
            <div className="mt-4 mb-4">
                ...
                <button disabled={isDisabled}
                    className="w-14 h-8 bg-green-500 ms-4 disabled:bg-gray-400 text-white"
                    onClick={addTask}>
                    Add
                </button>
                {tasks.includes(newTask) && (
                    <span className='text-yellow-200 mt-4 '> Task must be unique</span>
                )}
            </div>
            <ol>
                {tasks.map((task, index) =>
                    <li key={task}>
                        ...
                        <button className="mb-2 ms-1 p-1 bg-red-500 text-white "
                            onClick={() => deleteTask(index)}>
                            Delete
                        </button>
                        <button className="h-8 ms-2 bg-orange-500"
